...
namespace UnitTestIntegralProject
{
    ...
    public class UnitTestIntegral
    {
        ...
        [TestMethod]
        [Description("Убеждаемся, что синхронный метод вычисления интеграла и параллельный дают одинаковые результаты.")]
        public void TestIntegralSyncAndParallel()
        {
            const double edge = 1000;
            const double epsilon = 0.0001;

            double s1 = Integral.Get(x => 1.0 / (1.0 + x * x), -edge, edge);
            double s2 = Integral.GetParallel(x => 1.0 / (1.0 + x * x), -edge, edge);

            Assert.IsTrue(Math.Abs(s1 - s2) < epsilon);
        }

        [TestMethod]
        [Description("Убеждаемся, что паралльное вычисление интеграла работает быстрее.")]
        public void TestIntegralPerformance()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                sw2.Start();
                double s2 = Integral.GetParallel(x => 1.0 / (1.0 + x * x), -edge, edge, split);
                sw2.Stop();
            }
